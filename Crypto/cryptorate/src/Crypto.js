      ...
      .then((res) => {
        ...
        this.setState((state) => {
          ...
          for (const [ticker, cryptoRate] of Object.entries(tickers)) {
            let lastCrypotoObj = state.cryptoList.find((cryptoObj) => {
            ...
            if (lastCrypotoObj !== undefined) {
              if (newCryptoObj.lastRate > lastCrypotoObj.lastRate) {
                newCryptoObj.cssClass = "green";
                newCryptoObj.htmlArray = String.fromCharCode(8593);
              } else if (newCryptoObj.lastRate < lastCrypotoObj.lastRate) {
